...
public class ClientThread extends Thread {
  ...
  private void handleRequest(final HttpRequest request, final BufferedOutputStream bufferedWriter)
    ...
    if (routeKey.equals("GET/camera/stream")) {

      handleMultipart(bufferedWriter);

    } else if (!matchedRoutes.isEmpty()) {
  ...
  private void handleMultipart(final BufferedOutputStream bufferedWriter) throws IOException {

    ResponseWriter.writeResponse(bufferedWriter,

        new HttpResponse.Builder().setStatusCode(200)

            .addHeader("Content-Type", "multipart/x-mixed-replace; boundary=\"OSMZ_boundary\"")
    ...
    while (true) {
      ...
      if (MainActivity.img != null) {
        ...
        byte[] photoBytes = MainActivity.img;

        bufferedWriter.write("--OSMZ_boundary\n".getBytes(StandardCharsets.UTF_8));

        bufferedWriter.write("Content-Type: image/jpeg\n".getBytes(StandardCharsets.UTF_8));

        bufferedWriter.write("\n".getBytes(StandardCharsets.UTF_8));

        bufferedWriter.write(photoBytes);

        bufferedWriter.write("\n".getBytes(StandardCharsets.UTF_8));

        bufferedWriter.flush();

        MainActivity.taken = true;
